...
public abstract class Minecraft implements Runnable {
	...
	public String debugInfoPoz() {
		return "X: " + Math.round(this.thePlayer.posX) + " Y: " + Math.round(this.thePlayer.boundingBox.minY) + " Z: " + Math.round(this.thePlayer.posZ);
	}
	...
	public void startGame() throws LWJGLException {
		...
		Display.setTitle("Minecraft Alpha+ 1.0.12");
	...
	public void run() {
		...
		try {
			...
			this.displayUnexpectedThrowable(new UnexpectedThrowable("Failed to start game", exception7));
			return;
		}
		...
		try {
			...
			while(this.running && (this.mcApplet == null || this.mcApplet.isActive())) {
				...
				for(int i10 = 0; i10 < this.timer.elapsedTicks; ++i10) {
					++this.ticksRan;

					try {
						this.runTick();
					} catch (MinecraftException minecraftException6) {
						this.theWorld = null;
						this.changeWorld1((World)null);
		...
		} catch (MinecraftError minecraftError8) {
		} catch (Throwable throwable9) {
			this.theWorld = null;
			throwable9.printStackTrace();
			this.displayUnexpectedThrowable(new UnexpectedThrowable("Unexpected error", throwable9));
		}
	...
	private void sendClickBlockToController(int button, boolean clicked) {
		...
		if(!this.playerController.isInTestMode && (button != 0 || this.leftClickCounter <= 0)) {
			if(clicked && this.objectMouseOver != null && this.objectMouseOver.typeOfHit == 0 && button == 0) {
				...
				Block block6 = Block.blocksList[this.theWorld.getBlockId(i3, i4, i5)];
				int i7 = (int)MathHelper.abs((float)(i3 - this.theWorld.spawnX));
				int i8 = (int)MathHelper.abs((float)(i5 - this.theWorld.spawnZ));
				if(i7 > i8) {
					i8 = i7;
				}

				if(block6 != Block.bedrock && (!this.theWorld.multiplayerWorld || i8 > 16) || this.thePlayer.unusedMiningCooldown >= 100) {
	...
	private void clickMouse(int button) {
		if(button != 0 || this.leftClickCounter <= 0) {
			...
			if(this.objectMouseOver == null) {
				if(button == 0 && this.playerController.hasMissTime()) {
				...
				int i7 = (int)MathHelper.abs((float)(i3 - this.theWorld.spawnX));
				int i8 = (int)MathHelper.abs((float)(i4 - this.theWorld.spawnZ));
				if(i7 > i8) {
					i8 = i7;
				}
				...
				if(button == 0) {
					...
					int i10 = 0;
					if(!this.theWorld.multiplayerWorld || i8 > 16 || this.thePlayer.unusedMiningCooldown >= 100) {
						if(itemStack9 != null) {
							i10 = itemStack9.stackSize;
						}

						if(this.playerController.onPlayerRightClick(this.thePlayer, this.theWorld, itemStack9, i3, i2, i4, i5)) {
							this.thePlayer.swingItem();
						}
					}
					...
					} else if(itemStack9.stackSize != i10 || this.playerController.hasInfiniteItems()) {
			...
			if(button == 1) {
				...
				if(itemStack11 != null) {
					i2 = itemStack11.stackSize;
					ItemStack itemStack12 = itemStack11.useItemRightClick(this.theWorld, this.thePlayer);
					if(itemStack12 != itemStack11 || itemStack12 != null && itemStack12.stackSize != i2) {
						this.thePlayer.inventory.mainInventory[this.thePlayer.inventory.currentItem] = itemStack12;
	...
	private void clickMiddleMouseButton() {
		if(this.objectMouseOver != null) {
			...
			this.thePlayer.inventory.setCurrentItem(i1, this.playerController.instabuild);
		}
									...
									while(true) {
										...
										if(Keyboard.getEventKey() == Keyboard.KEY_F11) {
											...
											if(this.currentScreen != null) {
												...
												if(this.options.creativeMode && Keyboard.getEventKey() == this.options.keyBindCreative.keyCode) {
													this.displayGuiScreen(new GuiInventoryCreative());
												}
											...
											for(i3 = 0; i3 < 9; ++i3) {
												if(Keyboard.getEventKey() == Keyboard.KEY_1 + i3) {
	...
	public void installResource(String name, File file) {
		...
		if(string4.equalsIgnoreCase("sound")) {
			...
			this.sndManager.addMusic(name, file);
		} else if(string4.equalsIgnoreCase("loops")) {
			this.sndManager.addToLoopsPool(name, file);
		}
	...
	public static void startMainThread(String username, String sessionId, String server) {
		boolean z3 = false;
		Frame frame4 = new Frame("Minecraft");
		...
		minecraftImpl6.creativeMode = z3;
