...
public class RenderGlobal implements IWorldAccess {
	...
	public void renderEntities(Vec3D vector, ICamera camera, float renderPartialTick) {
		if(this.renderEntitiesStartupCounter > 0) {
			...
			for(i6 = 0; i6 < list5.size(); ++i6) {
				...
				if(entity7.isInRangeToRenderVec3D(vector) && camera.isBoundingBoxInFrustum(entity7.boundingBox) && (entity7 != this.mc.thePlayer || this.mc.options.thirdPersonView > 0)) {
	...
	public int sortAndRender(EntityPlayer entityPlayer1, int i2, double d3) {
		...
		if(this.occlusionEnabled && !this.mc.options.anaglyph && i2 == 0) {
			...
			i18 = b17 + this.renderSortedRenderers(b19, i20, i2, d3);
				...
				for(int i25 = i21; i25 < i20; ++i25) {
					if(this.sortedWorldRenderers[i25].skipAllRenderPasses()) {
						...
						if(this.sortedWorldRenderers[i25].isInFrustum && !this.sortedWorldRenderers[i25].isWaitingOnOcclusionQuery) {
							float f26 = MathHelper.sqrt_float(this.sortedWorldRenderers[i25].distanceToEntitySquared(entityPlayer1));
			...
			i18 = b17 + this.renderSortedRenderers(0, this.sortedWorldRenderers.length, i2, d3);
		}
	...
	public void renderSky(float renderPartialTick) {
		...
		if(this.mc.options.anaglyph) {
			float f8 = (f3 * 30.0F + f4 * 59.0F + f5 * 11.0F) / 100.0F;
			f6 = (f3 * 30.0F + f4 * 70.0F) / 100.0F;
			f7 = (f3 * 30.0F + f5 * 70.0F) / 100.0F;
			f3 = f8;
			f4 = f6;
			f5 = f7;
		}
	...
	public void drawBlockBreaking(EntityPlayer entityPlayer1, MovingObjectPosition movingObjectPosition2, int i3, ItemStack itemStack4, float f5) {
		...
		if(i3 == 0) {
			if(this.damagePartialTime > 0.0F) {
				...
				GL11.glPushMatrix();
				i7 = this.theWorld.getBlockId(movingObjectPosition2.blockX, movingObjectPosition2.blockY, movingObjectPosition2.blockZ);
			...
			i7 = this.renderEngine.getTexture("/terrain.png");
	...
	public void playRecord(String record, int x, int y, int z) {
		if(record != null) {
			if(record.equalsIgnoreCase("magnet")) {
				this.mc.ingameGUI.setRecordPlayingMessage("Markus Alexei - Magnet");
			}

			if(record.equalsIgnoreCase("13")) {
				this.mc.ingameGUI.setRecordPlayingMessage("C418 - 13");
			}

			if(record.equalsIgnoreCase("cat")) {
				this.mc.ingameGUI.setRecordPlayingMessage("C418 - Cat");
			}

			if(record.equalsIgnoreCase("stal")) {
				this.mc.ingameGUI.setRecordPlayingMessage("C418 - Stal");
			}
		}
	...
	public void makeAmbience(String string1, double d2, double d4, double d6, float f8, float f9) {
	}

	public void updateAmbience(String string1, double d2, double d4, double d6, float f8, float f9) {
	}
