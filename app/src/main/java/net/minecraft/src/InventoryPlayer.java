...
public class InventoryPlayer implements IInventory {
	...
	public void replaceBlock(Block block1) {
		if(block1 != null) {
			int i2 = this.getInventorySlotContainItem(block1.blockID);
			if(i2 >= 0) {
				int i3 = this.mainInventory[i2].stackSize;
				this.mainInventory[i2] = this.mainInventory[this.currentItem];
				this.mainInventory[this.currentItem] = new ItemStack(block1, i3);
			} else {
				this.mainInventory[this.currentItem] = new ItemStack(block1);
			}
		}
