...
final class CacheLineSize {
	...
	static int getCacheLineSize() {
		...
		final int MAX_SIZE = LWJGLUtil.getPrivilegedInteger("org.lwjgl.util.mapped.CacheLineMaxSize", 1024) / 4; // in #
																													// of
																													// integers
		// Detection will stop when the execution time increases by more than
		// CacheLineTimeThreshold %.
		final double TIME_THRESHOLD = 1.0
				+ LWJGLUtil.getPrivilegedInteger("org.lwjgl.util.mapped.CacheLineTimeThreshold", 50) / 100.0;
		...
		try {
			...
			for (int i = MAX_SIZE; i >= 1; i >>= 1) {
				...
				if (totalTime > 0) { // Ignore first run
					...
					if ((double) time / (double) avgTime > TIME_THRESHOLD) { // Try to detect a noticeable jump in
																				// execution time
						cacheLineSize = (i << 1) * 4;
						found = true;
						break;
	...
	private static IntBuffer getMemory(final int START_SIZE) {
		...
		final ByteBuffer buffer = ByteBuffer.allocateDirect((START_SIZE * 4) + PAGE_SIZE)
				.order(ByteOrder.nativeOrder());

		// Align to page and, consequently, to cache line. Otherwise results will be
		// inconsistent.
	...
	private static long doTest(final int threads, final int repeats, final int padding, final IntBuffer memory,
			final ExecutorCompletionService<Long> completionService) {
		for (int i = 0; i < threads; i++)
			submitTest(completionService, i, repeats, memory, padding);
		return waitForResults(threads, completionService);
	}

	private static void submitTest(final ExecutorCompletionService<Long> completionService, final int index,
			final int repeats, final IntBuffer memory, final int padding) {
