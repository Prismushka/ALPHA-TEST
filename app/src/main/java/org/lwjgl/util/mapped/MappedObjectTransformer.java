...
public class MappedObjectTransformer {
	...
	static final Map<String, MappedObjectTransformer.MappedSubtypeInfo> className_to_subtype;
	...
	public static void register(Class<? extends MappedObject> type) {
		if(!MappedObjectClassLoader.FORKED) {
			...
			if(mapped != null && mapped.padding() < 0) {
				...
				long sizeof = 0L;
				Field[] align = type.getDeclaredFields();
				int padding = align.length;

				for(int cacheLinePadded = 0; cacheLinePadded < padding; ++cacheLinePadded) {
					Field mappedType = align[cacheLinePadded];
					MappedObjectTransformer.FieldInfo fieldInfo = registerField(mapped == null || mapped.autoGenerateOffsets(), className, sizeof, mappedType);
					if(fieldInfo != null) {
						fields.put(mappedType.getName(), fieldInfo);
				...
				if(mapped != null) {
					...
					if(mapped.cacheLinePadding()) {
						...
						int var13 = (int)(sizeof % (long)CacheUtil.getCacheLineSize());
						if(var13 != 0) {
							padding = CacheUtil.getCacheLineSize() - var13;
						}
				...
				sizeof += (long)padding;
				MappedObjectTransformer.MappedSubtypeInfo var14 = new MappedObjectTransformer.MappedSubtypeInfo(className, fields, (int)sizeof, var11, padding, var12);
				if(className_to_subtype.put(className, var14) != null) {
	...
	private static CheckClassAdapter getMethodGenAdapter(final String className, final ClassVisitor cv) {
		return new CheckClassAdapter(cv) {
			public void visitEnd() {
				...
				this.generateNext();
				Iterator i$ = mappedSubtype.fields.keySet().iterator();
	...
	static int transformMethodCall(InsnList instructions, int i, Map<AbstractInsnNode, Frame<BasicValue>> frameMap, MethodInsnNode methodInsn, MappedObjectTransformer.MappedSubtypeInfo mappedType, Map<Integer, MappedObjectTransformer.MappedSubtypeInfo> arrayVars) {
		switch(methodInsn.getOpcode()) {
		case 182:
			if("asArray".equals(methodInsn.name) && methodInsn.desc.equals("()[L" + MAPPED_OBJECT_JVM + ";")) {
				AbstractInsnNode isMapDirectMethod1 = methodInsn.getNext();
				checkInsnAfterIsArray(isMapDirectMethod1, 192);
				isMapDirectMethod1 = isMapDirectMethod1.getNext();
				checkInsnAfterIsArray(isMapDirectMethod1, 58);
				Frame isMapBufferMethod1 = (Frame)frameMap.get(isMapDirectMethod1);
				String isMallocMethod1 = ((BasicValue)isMapBufferMethod1.getStack(isMapBufferMethod1.getStackSize() - 1)).getType().getElementType().getInternalName();
				if(!methodInsn.owner.equals(isMallocMethod1)) {
					throw new ClassCastException("Source: " + methodInsn.owner + " - Target: " + isMallocMethod1);
				}

				VarInsnNode varInstruction = (VarInsnNode)isMapDirectMethod1;
	...
	private static InsnList generateByteBufferInstructions(FieldInsnNode fieldInsn, MappedObjectTransformer.MappedSubtypeInfo mappedSubtype, long fieldOffset) {
		...
		if(fieldInsn.getOpcode() == 180) {
			...
			list.add((AbstractInsnNode)(new LdcInsnNode(Long.valueOf(((MappedObjectTransformer.FieldInfo)mappedSubtype.fields.get(fieldInsn.name)).length))));
	...
	static int transformArrayAccess(InsnList instructions, int i, Map<AbstractInsnNode, Frame<BasicValue>> frameMap, VarInsnNode loadInsn, MappedObjectTransformer.MappedSubtypeInfo mappedSubtype, int var) {
		int loadStackSize = ((Frame)frameMap.get(loadInsn)).getStackSize() + 1;
		Object nextInsn = loadInsn;

		while(true) {
			nextInsn = ((AbstractInsnNode)nextInsn).getNext();
			if(nextInsn == null) {
				throw new InternalError();
			}

			Frame frame = (Frame)frameMap.get(nextInsn);
			if(frame != null) {
				int stackSize = frame.getStackSize();
				if(stackSize == loadStackSize + 1 && ((AbstractInsnNode)nextInsn).getOpcode() == 50) {
					Object aaLoadInsn = nextInsn;
					...
					while(true) {
						...
						if(frame != null) {
							...
							if(stackSize == loadStackSize + 1 && ((AbstractInsnNode)nextInsn).getOpcode() == 181) {
								fieldInsn = (FieldInsnNode)nextInsn;
								instructions.insert((AbstractInsnNode)nextInsn, (AbstractInsnNode)(new MethodInsnNode(184, mappedSubtype.className, setterName(fieldInsn.name), "(L" + mappedSubtype.className + ";I" + fieldInsn.desc + ")V")));
								instructions.remove((AbstractInsnNode)nextInsn);
								break;
							}

							if(stackSize == loadStackSize && ((AbstractInsnNode)nextInsn).getOpcode() == 180) {
								fieldInsn = (FieldInsnNode)nextInsn;
								instructions.insert((AbstractInsnNode)nextInsn, (AbstractInsnNode)(new MethodInsnNode(184, mappedSubtype.className, getterName(fieldInsn.name), "(L" + mappedSubtype.className + ";I)" + fieldInsn.desc)));
								instructions.remove((AbstractInsnNode)nextInsn);
								break;
							}

							if(stackSize == loadStackSize && ((AbstractInsnNode)nextInsn).getOpcode() == 89 && ((AbstractInsnNode)nextInsn).getNext().getOpcode() == 180) {
								fieldInsn = (FieldInsnNode)((AbstractInsnNode)nextInsn).getNext();
								...
								instructions.insert(((AbstractInsnNode)nextInsn).getNext(), (AbstractInsnNode)getter);
								instructions.remove((AbstractInsnNode)nextInsn);
								instructions.remove(fieldInsn);
								nextInsn = getter;
							} else if(stackSize < loadStackSize) {
								throw new ClassFormatError("Invalid " + mappedSubtype.className + " view array usage detected: " + getOpcodeName((AbstractInsnNode)nextInsn));
							}
						}
					}
				...
				if(stackSize == loadStackSize && ((AbstractInsnNode)nextInsn).getOpcode() == 190) {
					...
					instructions.remove((AbstractInsnNode)nextInsn);
					loadInsn.var = var;
					instructions.insert(loadInsn, (AbstractInsnNode)(new MethodInsnNode(182, mappedSubtype.className, "capacity", "()I")));
					return i + 1;
				}
		...
		try {
			...
			for(int i$ = 0; i$ < len$; ++i$) {
				...
				if(Modifier.isStatic(field.getModifiers()) && field.getType() == Integer.TYPE) {
					...
					int len$1 = prefixFilters.length;

					for(int i$1 = 0; i$1 < len$1; ++i$1) {
						String filter = arr$[i$1];
	...
	static AbstractInsnNode getIntNode(int value) {
		return (AbstractInsnNode)(value <= 5 && -1 <= value ? new InsnNode(2 + value + 1) : (value >= -128 && value <= 127 ? new IntInsnNode(16, value) : (value >= Short.MIN_VALUE && value <= Short.MAX_VALUE ? new IntInsnNode(17, value) : new LdcInsnNode(Integer.valueOf(value)))));
	}
	...
	private static class TransformationAdapter extends CheckClassAdapter {
				...
				return (FieldVisitor)((access & 8) == 0 ? new FieldNode(access, name, desc, signature, value) {
					public void visitEnd() {
						if(this.visibleAnnotations == null) {
							...
							boolean after = false;
							byte byteLength = 0;
							Iterator count = this.visibleAnnotations.iterator();

							while(count.hasNext()) {
								AnnotationNode i = (AnnotationNode)count.next();
								if(MappedObjectTransformer.CACHE_LINE_PAD_JVM.equals(i.desc)) {
									...
									after = true;
									if(i.values != null) {
										for(int i1 = 0; i1 < i.values.size(); i1 += 2) {
											boolean value = i.values.get(i1 + 1).equals(Boolean.TRUE);
											if("before".equals(i.values.get(i1))) {
												before = value;
											} else {
												after = value;
											}
							...
							int var9;
							if(before) {
								var8 = CacheUtil.getCacheLineSize() / byteLength - 1;

								for(var9 = var8; var9 >= 1; --var9) {
									TransformationAdapter.this.cv.visitField(this.access | 1 | 4096, this.name + "$PAD_" + var9, this.desc, this.signature, (Object)null);
								}
							}
							...
							if(after) {
								...
								for(var9 = 1; var9 <= var8; ++var9) {
									TransformationAdapter.this.cv.visitField(this.access | 1 | 4096, this.name + "$PAD" + var9, this.desc, this.signature, (Object)null);
								}
							}
		...
		public MethodVisitor visitMethod(final int access, String name, final String desc, final String signature, final String[] exceptions) {
					if("<init>".equals(name)) {
						MappedObjectTransformer.MappedSubtypeInfo mv = (MappedObjectTransformer.MappedSubtypeInfo)MappedObjectTransformer.className_to_subtype.get(this.className);
						if(mv != null) {
							if(!"()V".equals(desc)) {
								throw new ClassFormatError(this.className + " can only have a default constructor, found: " + desc);
							}
		
							MethodVisitor mv1 = super.visitMethod(access, name, desc, signature, exceptions);
							mv1.visitVarInsn(25, 0);
							mv1.visitMethodInsn(183, MappedObjectTransformer.MAPPED_OBJECT_JVM, "<init>", "()V");
							mv1.visitInsn(177);
							mv1.visitMaxs(0, 0);
					...
					for(i = 0; i < instructions.size(); ++i) {
						...
						case 2:
							if(instruction.getOpcode() == 25) {
								VarInsnNode var11 = (VarInsnNode)instruction;
								MappedObjectTransformer.MappedSubtypeInfo var12 = (MappedObjectTransformer.MappedSubtypeInfo)arrayVars.get(Integer.valueOf(var11.var));
								if(var12 != null) {
									i = MappedObjectTransformer.transformArrayAccess(instructions, i, frameMap, var11, var12, var11.var);
								}
							}
						case 3:
						default:
